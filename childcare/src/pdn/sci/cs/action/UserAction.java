...
public class UserAction extends BaseAction {
	...
	public String signIn() {
		...
		if (hasErrors()) {
			...
			try {
				systemUser = systemUserService.signIn(username,
		...
		if (hasErrors()) {
			...
			if (systemUser == null) {
				...
				mm.sendMail("axio.callcenter@gmail.com",
						"vajirakarunathilake@gmail.com", "Testing123",
	...
	public String passwordChangeSave() throws Exception {
		...
		if (!newUserPassword.equals(newUserPasswordConfirm)) {
			...
			return INPUT;
				...
				PasswordEncryption.encrypt(oldUserPassword))) {
		...
		if (systemUser != null) {
			...
			if (hasErrors()) {
				...
				if (operationMode == OPERATION_MODE.ADD
					...
					systemUser.setUserPassword(PasswordEncryption
							.encrypt(systemUser.getUserPassword()));
					systemUser = systemUserService.save(systemUser);
				} else if (operationMode == OPERATION_MODE.EDIT
						&& !systemUser.getId().isEmpty()) {

					if (systemUser.getUserPassword() == null
							|| systemUser.getUserPassword().isEmpty()) {
						SystemUser existingUser = systemUserService
	...
	private void populateAddList() {
		...
		Province selectProvince = new Province("-101",
	...
	private void validateUser() {
		try {
			if (systemUser.getUsername().isEmpty()
					|| systemUser.getUsername().trim().length() == 0) {
				addFieldError("systemUser.username", "Username cannot be empty");
			}

			if (systemUser.getName().isEmpty()
					|| systemUser.getName().trim().length() == 0) {
				addFieldError("systemUser.name", "Name cannot be empty");
			}

			if (systemUser.getUserRole().equals(
					SystemUser.USER_ROLE.OFFICER.name())) {
				if (systemUser.getReferenceId().equals("-101")
						|| systemUser.getReferenceId() == null
						|| systemUser.getReferenceId().isEmpty()
						|| systemUser.getReferenceId().trim().length() == 0) {
					addFieldError("systemUser.referenceId",
			...
			if (systemUser.getUserRole().equals(
				...
				if (systemUser.getReferenceId() == null
						|| systemUser.getReferenceId().isEmpty()
						|| systemUser.getReferenceId().trim().length() == 0) {
					addFieldError("systemUser.referenceId",
							"Please select your work location");
			...
			if (operationMode != OPERATION_MODE.EDIT) {
				if (systemUserService
						.searchByUsername(systemUser.getUsername()) != null) {
					addFieldError("systemUser.username",
	...
	public void validateMobile(String mobile) {
		...
		if (check == false) {
			addFieldError("systemUser.mobile",
					"Number must have 10 digit numbers");
		}
	...
	public String search() {
		if (systemUser != null) {
			...
			try {
				...
				pager = systemUserService.search(systemUser.getName(),
						systemUser.getUserRole(), systemUser.getMobile(),
						pageStart, pageSize);
			} catch (Exception e) {
				e.printStackTrace();
				populateAddList();
	...
	public String changeProfileForm() {
		...
		if (getSessionUser().getPost().equals(
				UserPost.PROVINCIAL_OFFICER.getStatusCode())) {
			province = provinceService.findById(systemUser.getReferenceId())
					.getName();
		}
