...
public class LamaNivasaAction extends BaseAction {
	...
	public String searchForm() {
		if(isAdminOrMinistry()){
			...
			if(isProvincialCommissionerORisProbationOfficerOfHeadquarter()){
				...
				probationUnitList.add(probationUnit);
			}
		}
		return SUCCESS;
	}
	...
	private void fieldsGenerators() {
		lamaNivasa.setGramaSevakaDivision(
				gramaSevakaDivsionService.findById(lamaNivasa.getGramaSevakaDivision().getId()));
		lamaNivasa.setProbationUnit(probationUnitService.findById(lamaNivasa.getProbationUnit().getId()));
	}
	...
	public String dsSelectJson() {
		...
		if (divisionalSecretariatId != null) {
			gramaSevakaDivisionList = gramaSevakaDivsionService.findByDivisionalSecretariatId(divisionalSecretariatId);
		}
	...
	private void validateLamaNivasa() {
		...
		if (lamaNivasa.getGramaSevakaDivision() == null || lamaNivasa.getGramaSevakaDivision().getId().equals("-1")
		...
		// if(lamaNivasa.getProbationOfficer() == null ||
		// lamaNivasa.getProbationOfficer().equals("-1"))
		// {
		// addFieldError("lamaNivasa.probationOfficer", "Probation office should
		// be assigned");
		// }
	...
	private void viewInit() {
		...
		// gramaSevakaDivsionService.findByDivisionalSecretariatId("DS1");
		// probationUnitOfficerList =
		// systemUserService.findByUserRole(SystemUser.USER_ROLE.OFFICER);
	}
