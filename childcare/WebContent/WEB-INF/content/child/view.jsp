...
<div>
	...
	<table id="commonTable" class="blueTbl" width="100%">
		<tr>
			<th colspan="7"><s:text name="page.view.child.detail" /></th>
		</tr>
		<tr>
			<th><s:text name="page.view.lamanivasaya" /></th>
			<td colspan="5"><s:property value="child.lamaNivasa.name" /></td>
		</tr>
		...
		<tr>
			...
			<td><s:date name="child.dateOfBirth" format="yyyy/MM/dd" /></td>
			<th><s:text name="page.view.date.of.registration" /></th>
			<td><s:date name="child.dateOfRegistration" format="yyyy/MM/dd" />
			</td>
		</tr>
		<tr>
			...
			<td><s:property value="child.religion" /></td>
		</tr>
		<tr>
			...
			<td><s:property value="child.vaccinationCard" /></td>
		</tr>
		<tr>
			<th><s:text name="page.add.disable" /></th>
			<td><s:property value="child.disable" /></td>
			<th><s:text name="page.add.type.disability" /></th>
			<td colspan="3"><s:property value="child.typeOfDisability" /></td>
		</tr>
		<tr>
			...
			<td colspan="3"><s:property value="child.school" /></td>
		</tr>
		<tr>
			<th><s:text name="page.add.intake" /></th>
			<td colspan="3"><s:property value="child.intakeMethod" /></td>
			<th><s:text name="page.add.prob.unit" /></th>
			<td colspan="3"><s:property value="child.originalProbationUnit" />
			</td>
		</tr>
		<tr>
			<th><s:text name="page.add.comment" /></th>
			<td colspan="5"><s:property value="child.comment" /></td>
		</tr>

		<tr>
			<th><s:text name="page.add.history" /></th>
			<td colspan="5"><s:property value="child.history" /></td>
		</tr>

		<tr>
			<td colspan="2"><s:url var="childPictureFrameUrl" action="frame"
					...
					<input type="button"
						value="<s:text name="page.view.button.pictures"/>" />
				</sj:a> | <s:url var="childGuardianFrameUrl" action="frame"
					namespace="/childGuardian" includeParams="none">
					<s:param name="childId" value="child.id"></s:param>
				</s:url> <sj:a href="%{#childGuardianFrameUrl}" targets="childDetailViewDiv">
					<input type="button"
						value="<s:text name="page.view.button.guard"/>" />
				</sj:a> | <s:url var="childRecordFrameUrl" action="frame"
					namespace="/childRecord" includeParams="none">
					<s:param name="childId" value="child.id"></s:param>
				</s:url> <sj:a href="%{#childRecordFrameUrl}" targets="childDetailViewDiv">
					<input type="button"
						value="<s:text name="page.view.button.record"/>" />
				</sj:a> | <s:url var="childCareplanFrameUrl" action="frame"
			...
			<td><s:if test="probationOfficer || user">
				 ...
				 <s:if
					...
					<s:else>
						...
						<sj:a href="%{childRestoreUrl}" targets="childResultDiv"
							onClickTopics="/confirmRestore">
							<input type="button"
								value="<s:text name="page.view.button.restore"/>" />
						</sj:a>
					</s:else>
				</s:if></td>
