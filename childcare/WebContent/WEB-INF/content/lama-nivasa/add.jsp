...
<div style="margin: 10px 0 0 0px">
	...
	<s:form id="lamaNivasaAddForm" action="save" method="post"
		...
		<table id="contentTable" class="blueTbl" width="100%">
			...
			<tr>
				<th><s:text name="lamaNivasa.add.name" /><span
			...
			<tr>
				<th><s:text name="lamaNivasa.add.category" /><span
					class="required">*</span></th>
				<td><table>
			...
			<s:if test="!user">
					...
					<td colspan="5">
						<table>
							<sj:select href="%{districtSelectJsonUrl}" id="dsId"
								...
								cssStyle="width: 250px" onclick="myFunction()" />
				...
				<tr>
					<th><s:text name="lamaNivasa.add.gNDivision" /><span
						class="required">*</span></th>
					<td colspan="5">
						<table>
							<sj:select required="true" href="%{dsSelectJsonUrl}" id="gsId"
								...
								cssStyle="width: 250px" onclick="myFunction1()" />
				...
				<tr>
					<th><s:text name="lamaNivasa.add.unit" /><span
						class="required">*</span></th>
					<td colspan="2">
						<table>
							<sj:select href="%{districtSelectJsonUrl}"
								...
								cssStyle="width: 250px" onclick="myFunction2()" />
					...
					<td colspan="2">
						<table>
							<sj:select required="true" href="%{districtSelectJsonUrl}"
								...
								emptyOption="false" headerKey="-1"
								headerValue="Please Select Main Officer" cssStyle="width: 250px"
								onclick="myFunction3()" />
			...
			<tr>
				<th><s:text name="lamaNivasa.add.address" /><span
					class="required">*</span></th>
				<td colspan="5"><table>
			...
			<tr>
				<th><s:text name="lamaNivasa.add.telephone" /><span
					class="required">*</span></th>
				<td><table>
				...
				<td><table>
						<s:textfield name="lamaNivasa.fax" cssStyle="width:150px"
							maxlength="15" />
					</table></td>
				<th><s:text name="lamaNivasa.add.email" /></th>
				<td><table>
						<s:textfield name="lamaNivasa.email" cssStyle="width:200px"
							maxlength="60" />
					</table></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><table>
						<s:select name="lamaNivasa.registrationStatus"
							...
							listValue="listValue" cssStyle="width:150px" />
					</table></td>
			...
			<tr>
				<th></th>
				<td colspan="5"><table>
						<sj:submit key="lamaNivasa.add.submit.btn"
							targets="lamaNivasaResultDiv" />
					</table></td>
	...
	$(document)
			.ready(
					function() {
						...
						if (didadd != "") {
							$("#district").val(didadd).change();
						}

						setTimeout(function() {
							if (did != "") {
								$("#dsId").val(dsid).change();
							}
							if (dsidadd != "") {
								$("#dsId").val(dsidadd).change();
							}
						}, 200);
						...
						$('#lamaNivasa.probationUnit.id').change(function(){
							$('#demo3').html("");
						});
					});

	function myFunction() {
		//document.getElementById("demo").innerHTML = "You have to select a District first";
		if($('#district').val()==-1){
			$('#demo').html("Please select a district");
		}
	}
	function myFunction1() {
		//document.getElementById("demo1").innerHTML = "You have to select a Div.Secretariat first";
		if($('#dsId').val()==-1){
			$('#demo1').html("Please select a Div.Secretariat");
		}
	}
	function myFunction2() {
		//document.getElementById("demo2").innerHTML = "You have to select a District first";
		if($('#gsId').val()==-1){
			$('#demo2').html("Please select a Grama Niladari Division");
		}
	}
	function myFunction3() {
		//document.getElementById("demo3").innerHTML = "You have to select a District first";
		if($('#lamaNivasa.probationUnit.id').val()==-1){
			$('#demo3').html("Please select a Unit");
		}
	}
</script>
